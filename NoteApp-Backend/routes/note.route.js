...
noteRouter.post("/save", async (req, res) => {
    ...
    try {
        ...
        console.log(note);
        await note.save()
        res.status(201).send({note, message: "Note has been Saved..."})
    }
    catch(error) {
        res.status(500).send(error)
    }   
...
noteRouter.patch("/update", async (req, res) => {
    ...
    try {
        ...
        res.status(201).send({message: "Note has been Updated..."})
    }
    catch(error) {
        res.status(500).send(error)
    }   
...
noteRouter.delete("/delete", async (req, res) => {
    ...
    try {
        ...
        res.status(201).send({message: "Note has been Deleted..."})
    }
    catch(error) {
        res.status(500).send(error)
    }   
